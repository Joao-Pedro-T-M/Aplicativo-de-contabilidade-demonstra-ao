...
import '../widgets/drag_inventory_method_widget.dart';
...
const Color kFilledLightBg = Color(0xFFF2FAFD);
...
class _CoursePageState extends State<CoursePage> {
  ...
  bool _simuladoReady = false;
  bool _navigating = false;
  @override
  void didUpdateWidget(covariant CoursePage oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (oldWidget.userId != widget.userId || oldWidget.course.id != widget.course.id) {
      setState(() {
        _selectedIndex = 0;
        _progressChanged = false;
        _hideAppBarForSimulado = false;
        _simuladoReady = false;
      });
    }
  }

  void _setSimuladoReady() {
    if (!mounted) return;
    setState(() {
      _simuladoReady = true;
    });
  ...
  Widget build(BuildContext context) {
    final tabs = <Widget>[
      _AprenderTab(
        key: ValueKey('aprender-${widget.userId ?? 'guest'}-${widget.course.id}'),
        course: widget.course,
        userId: widget.userId,
        onProgressChanged: _notifyProgressChanged,
      ),
SimuladoTab(
  key: ValueKey('simulado-${widget.userId ?? 'guest'}-${widget.course.id}'),
  course: widget.course,
  userId: widget.userId,
  onUiStateChanged: _setSimuladoHeaderHidden,
  onLoaded: _setSimuladoReady,
),
      PerfilTab(
        key: ValueKey('perfil-${widget.userId ?? 'guest'}-${widget.course.id}'),
        course: widget.course,
        userId: widget.userId,
      ),
    ];
...
body: Stack(
  children: [
    ...
    if (_selectedIndex == 1 && !_simuladoReady)
...
/// ---------------------- Aba Aprender (sem o card Ativo/Passivo/PL) ----------------------
...
class _AprenderTabState extends State<_AprenderTab> {
  ...
  Future<void> _loadProgressFromStorage() async {
    try {
      ...
      try {
        ...
        if (appState != null && appState.cloudSyncEnabled == true) {
          cloudSyncEnabled = true;
        }
      } catch (_) {}

      final localFuture =
          ProgressStorage.getCompletedTrailIds(widget.course.id, userId: widget.userId);
      ...
      if (finished == true) {
        ...
        try {
          ...
          if (widget.userId != null &&
              ...
              appState.syncManager != null) {
            appState.syncManager!.markCompletedLocally(widget.course.id, trail.id);
          } else {
            if (kDebugMode) {
              debugPrint('Cloud sync disabled or not configured - progresso mantido localmente');
            }
          }
        } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    final visibleIndices = <int>[
      for (var i = 0; i < course.trails.length; i++) i,
    ];
    ...
    return Scaffold(
      body: ListView.builder(
        ...
        itemBuilder: (_, idx) {
          final trailIndex = visibleIndices[idx];
          ...
          return LayoutBuilder(
            builder: (context, constraints) {
              ...
              final Color background = filled ? kPrimaryColor : kCardColor;
              final Color borderColor = filled
                  ? const Color.fromARGB(255, 68, 156, 156)
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                                ...
                                children: [
                                  Text(
                                    ...
                                    style: TextStyle(
                                      ...
                                      color: _pressed
                                          ? Colors.white
                                          : (widget.filled ? Colors.white : kTextColor),
                                      height: 1.02,
                                    ),
                                    maxLines: 2,
                            ...
                            Icon(
                              Icons.chevron_right,
                              color: _pressed ? Colors.white : kPrimaryColor,
                            ),
                          ],
                        ),
                      ),
...
class _LessonPageState extends State<LessonPage> with TickerProviderStateMixin {
  ...
  bool _sheetIsInteracting = false;
...
bool _onScrollNotification(ScrollNotification notif) {
  if (_sheetIsInteracting) return false;
 ...
 itemBuilder: (context, pageIndex) {
  ...
  final bool hasExplanation =
      q.explanation != null && (q.explanation as String).trim().isNotEmpty;
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        q.explanation!,
                        style: TextStyle(fontSize: 15 * scale, color: kTextColor),
                      ),
                    ],
                  ),
                ),
        ...
        children: [
          ...
          Padding(
            ...
            child: DragMatchSumWidget(
              ...
              explanation: q.explanation,
              groups: q.groups,
              dreEntries: q.dreEntries,
              isDfcMode: q.isDfcMode,
              initialCash: q.initialCash,
              dfcFlows: q.dfcFlows,
              onSubmitted: (allCorrect) {
                setState(() {
                  answeredCorrectList[pageIndex] = allCorrect;
  ...
  if (q is InventoryMethodQuestion) {
    return SingleChildScrollView(
      padding: EdgeInsets.zero,
      physics: const AlwaysScrollableScrollPhysics(),
      child: Column(
        children: [
          ...
          Padding(
            padding: EdgeInsets.symmetric(horizontal: 15.0 * scale),
           child: DragMatchInventoryMethodWidget.fromQuestion(
  q,
  onSubmitted: (allCorrect) {
    setState(() {
      answeredCorrectList[pageIndex] = allCorrect;
  ...
  onInteractionChanged: (isInteracting) {
    if (!mounted) return;
    setState(() {
      _sheetIsInteracting = isInteracting;
    });
  },
),        ),
          _animatedReveal(
            show: showExplanation,
            id: 'inventory-explanation-$pageIndex',
            child: Padding(
              padding: EdgeInsets.only(top: 22 * scale),
              child: _flatPanel(
                backgroundColor: kCardColor,
